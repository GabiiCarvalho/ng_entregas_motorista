  ...
  const handleLogin = async () => {
    ...
    try {
      const res = await authApi.login(
        telefone.replace(/\D/g, ''), senha);
      if (res.ok) {
        setMotorista(res.motorista);
        // Sempre faz verificação facial no login
...
const styles = StyleSheet.create({
  ...
  logoBox: {
    width: 56, height: 56, backgroundColor: Colors.primarySoft,
    borderRadius: 16, alignItems: 'center', justifyContent: 'center',
    marginTop: 32, marginBottom: Spacing.xl,
  },
  logoText: { fontSize: 22, fontWeight: '900', color: Colors.primary },
  title: { fontSize: Typography['3xl'], fontWeight: '800',
